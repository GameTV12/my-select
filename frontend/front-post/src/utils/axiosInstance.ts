...
const refreshTokenFn = async () => {
    try {
        ...
        if (access_token) {
            ...
            cookies.set('myselect_access', access_token, {expires: accessTime, domain: 'http://localhost:3000', path: '/'})
            cookies.set('myselect_refresh', refresh_token, {expires: refreshTime, domain: 'http://localhost:3000', path: '/'})
        }
...
axiosInstance.interceptors.request.use(
    async (config) => {
        ...
        console.log('Refresh token from post - ' + cookies.get('myselect_refresh'))
